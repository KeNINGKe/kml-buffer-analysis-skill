...
input_excel = os.path.join(base_path, '建筑统计_v3.xlsx')
output_excel = os.path.join(base_path, '建筑统计_人口岗位密度_final_v2.xlsx')
...
for index, row in df.iterrows():
    ...
    df.at[index, 'buffer面积'] = total_area
...
print(f"\n统计结果预览:")
print(df[['buffer_name', '人口数', '岗位数', 'buffer面积', '人岗密度']].head())
